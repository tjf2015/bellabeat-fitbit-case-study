...
ggplot(avg_steps_weekday, aes(x = weekday, y = avg_steps, fill = weekday)) +
...
ggplot(activity_sleep2, aes(x = total_steps, y = total_hours_asleep)) +
...
ggplot(intensity_long, aes(x = weekday, y = hours, fill = intensity)) +
  geom_col() +
  scale_y_continuous(breaks = seq(0, y_lim, 0.5)) +
...
ggplot(pct_10k, aes(x = is_weekend, y = pct, fill = is_weekend)) +
  geom_col(show.legend = FALSE) +
...
ggplot(daily_activity, aes(x = total_steps, color = is_weekend)) +
  stat_ecdf(size = 1) +
